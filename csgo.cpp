...
bool CSGO::init() {
	...
	// classes by sig.
	m_move_helper = pattern::find(m_client_dll, ("8B 0D ? ? ? ? 8B 46 08 68")).add(2).get< IMoveHelper* >(2);
	m_cl = **reinterpret_cast<CClientState***> ((*reinterpret_cast<uintptr_t**> (m_engine))[14] + 0x1);
	m_game = pattern::find(m_engine_dll, ("A1 ? ? ? ? B9 ? ? ? ? FF 75 08 FF 50 34")).add(1).get< CGame* >();
	m_render = pattern::find(m_engine_dll, ("A1 ? ? ? ? B9 ? ? ? ? FF 75 0C FF 75 08 FF 50 0C")).add(1).get< CRender* >();
	m_shadow_mgr = pattern::find(m_client_dll, ("A1 ? ? ? ? FF 90 ? ? ? ? 6A 00")).add(1).get().as< IClientShadowMgr* >();
	m_view_render = pattern::find(m_client_dll, ("8B 0D ? ? ? ? 8B 01 FF 50 4C 8B 06")).add(2).get< CViewRender* >(2);
	m_hud = pattern::find(m_client_dll, ("B9 ? ? ? ? 0F 94 C0 0F B6 C0 50 68")).add(0x1).get().as< CHud* >();
	m_gamerules = pattern::find(m_client_dll, ("8B 0D ? ? ? ? E8 ? ? ? ? 84 C0 75 6B")).add(0x2).get< C_CSGameRules* >();
	m_beams = pattern::find(m_client_dll, ("8D 04 24 50 A1 ? ? ? ? B9")).add(0x5).get< IViewRenderBeams* >();
	m_mem_alloc = PE::GetExport(m_tier0_dll, HASH("g_pMemAlloc")).get< IMemAlloc* >();
	GetGlowObjectManager = pattern::find(m_client_dll, ("A1 ? ? ? ? A8 01 75 4B")).as< GetGlowObjectManager_t >();
	m_glow = GetGlowObjectManager();
	m_hookable_cl = (void*)(uintptr_t(m_cl) + 0x8);
	...
	// functions.
	MD5_PseudoRandom = pattern::find(m_client_dll, ("55 8B EC 83 E4 F8 83 EC 70 6A 58")).as< MD5_PseudoRandom_t >();
	SetAbsAngles = pattern::find(m_client_dll, ("55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1 E8"));
	InvalidateBoneCache = pattern::find(m_client_dll, ("80 3D ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 81"));
	LockStudioHdr = pattern::find(m_client_dll, ("55 8B EC 51 53 8B D9 56 57 8D B3"));
	SetAbsOrigin = pattern::find(m_client_dll, ("55 8B EC 83 E4 F8 51 53 56 57 8B F1"));
	IsBreakableEntity = pattern::find(m_client_dll, ("55 8B EC 51 56 8B F1 85 F6 74 68 83 BE")).as< IsBreakableEntity_t >();
	SetAbsVelocity = pattern::find(m_client_dll, ("55 8B EC 83 E4 F8 83 EC 0C 53 56 57 8B 7D 08 8B F1"));
	AngleMatrix = pattern::find(m_client_dll, ("E8 ? ? ? ? 8B 07 89 46 0C")).rel32(0x1).as< AngleMatrix_t >();
	ComputeHitboxSurroundingBox = pattern::find(m_client_dll, ("E9 ? ? ? ? 32 C0 5D")).rel32(0x1);
	GetSequenceActivity = pattern::find(m_client_dll, ("53 56 8B F1 8B DA 85 F6 74 55"));
	LoadFromBuffer = pattern::find(m_client_dll, ("E8 ? ? ? ? 88 44 24 0F 8B 56 FC")).rel32(0x1).as< LoadFromBuffer_t >();
	ServerRankRevealAll = pattern::find(m_client_dll, ("55 8B EC 8B 0D ? ? ? ? 68")).as< ServerRankRevealAll_t >();
	HasC4 = pattern::find(m_client_dll, ("E8 ? ? ? ? 38 83")).rel32(0x1);
	InvalidatePhysicsRecursive = pattern::find(m_client_dll, ("E8 ? ? ? ? 89 5E 18")).rel32(0x1);
	IsReady = pattern::find(m_client_dll, ("E8 ? ? ? ? 59 C2 08 00 51 E8")).rel32(0x1).as< IsReady_t >();
	ShowAndUpdateSelection = pattern::find(m_client_dll, ("E8 ? ? ? ? A1 ? ? ? ? F3 0F 10 40 ? C6 83")).rel32(0x1).as< ShowAndUpdateSelection_t >();
	GetEconItemView = pattern::find(m_client_dll, ("8B 81 ? ? ? ? 81 C1 ? ? ? ? FF 50 04 83 C0 40 C3")).as< GetEconItemView_t >();
	GetStaticData = pattern::find(m_client_dll, ("55 8B EC 51 56 57 8B F1 E8 ? ? ? ? 0F B7 8E")).as< GetStaticData_t >();
	TEFireBullets = pattern::find(m_client_dll, ("C7 05 ? ? ? ? ? ? ? ? C7 05 ? ? ? ? ? ? ? ? 66 A3")).add(0x2).to();
	BeamAlloc = pattern::find(m_client_dll, ("E8 ? ? ? ? 8B F0 85 F6 74 7C")).rel32< BeamAlloc_t >(0x1);
	SetupBeam = pattern::find(m_client_dll, ("E8 ? ? ? ? 8B 07 33 C9")).rel32< SetupBeam_t >(0x1);
	ClearNotices = pattern::find(m_client_dll, ("E8 ? ? ? ? 68 ? ? ? ? B9 ? ? ? ? E8 ? ? ? ? 8B F0 85 F6 74 19")).rel32< ClearNotices_t >(0x1);
	AddListenerEntity = pattern::find(m_client_dll, ("55 8B EC 8B 0D ? ? ? ? 33 C0 56 85 C9 7E 32 8B 55 08 8B 35")).as< AddListenerEntity_t >();
	GetShotgunSpread = pattern::find(m_client_dll, ("E8 ? ? ? ? EB 38 83 EC 08")).rel32< GetShotgunSpread_t >(1);
	BoneAccessor = pattern::find(m_client_dll, ("8D 81 ? ? ? ? 50 8D 84 24")).add(2).to< size_t >();
	AnimOverlay = pattern::find(m_client_dll, ("8B 80 ? ? ? ? 8D 34 C8")).add(2).to< size_t >();
	SpawnTime = pattern::find(m_client_dll, ("F3 0F 5C 88 ? ? ? ? 0F")).add(4).to< size_t >();
	IsLocalPlayer = pattern::find(m_client_dll, ("74 ? 8A 83 ? ? ? ? 88")).add(4).to< size_t >();
	PlayerAnimState = pattern::find(m_client_dll, ("8B 8E ? ? ? ? 85 C9 74 3E")).add(2).to< size_t >();
	studioHdr = pattern::find(m_client_dll, ("8B 86 ? ? ? ? 89 44 24 10 85 C0")).add(2).to< size_t >();
	UTIL_TraceLine = pattern::find(m_client_dll, ("55 8B EC 83 E4 F0 83 EC 7C 56 52"));
	CTraceFilterSimple_vmt = UTIL_TraceLine.add(0x3D).to();
	CTraceFilterSkipTwoEntities_vmt = pattern::find(m_client_dll, ("E8 ? ? ? ? F3 0F 10 84 24 ? ? ? ? 8D 84 24 ? ? ? ? F3 0F 58 44 24")).rel32(1).add(0x59).to();
...
#ifdef _DEBUG
	Address debugbreak = pattern::find(g_csgo.m_client_dll, ("CC F3 0F 10 4D ? 0F 57 C0"));
