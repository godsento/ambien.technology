...
void InputPrediction::run() {
	...
	SetUserCmd(&pPredictedCmd, g_cl.m_cmd);
	...
	*g_csgo.m_nPredictionRandomSeed = g_cl.m_cmd->m_random_seed;
	...
	repredict(pMoveData, &pPredictedCmd);
}

void InputPrediction::repredict(CMoveData data, CUserCmd* cmd) {
	...
	g_cl.m_local->m_pCurrentCommand() = cmd;
	g_cl.m_local->m_PlayerCommand() = *cmd;

	// set predicted seed and player.
