...
bool penetration::runcustom(PenetrationInput_t* in, PenetrationOutput_t* out, vec3_t custom_origin) {
	static CTraceFilterSimple_game filter{};
	...
	while (damage > 0.f) {
		...
		// setup ray and trace.
		UTIL_TraceLineIgnoreTwoEntities(start, end, MASK_SHOT_HULL | CONTENTS_HITBOX, g_cl.m_local, in->m_target, COLLISION_GROUP_NONE, &trace);
