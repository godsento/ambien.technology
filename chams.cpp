...
void Chams::RenderPlayer(Player* player) {
	...
	if (g_menu.main.players.chams_fake.get()) {
		...
		if (player->m_bIsLocalPlayer() && ((g_input.GetKeyState(g_menu.main.misc.fakewalk.get()) && g_menu.main.misc.fakewalk_mode.get() == 1) || g_hvh.m_desync)) {
			...
			if (g_menu.main.players.chams_local_scope.get() && player->m_bIsScoped())
				SetAlpha((g_menu.main.players.chams_fake_blend.get() / 100.f) * scope_blend_percent);
